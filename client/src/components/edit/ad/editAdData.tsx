...
export function EditAdData() {
  ...
  //Fetch data function (with edit in fetch link)
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:4000/api/v1/ad/find/edit/${params.id}`,
        {
          headers: {
      ...
      setTitle(response.data.ad.title);
      setCondition(response.data.ad.condition);
      setCountry(response.data.ad.country);
      setMake(response.data.ad.make);
      setModel(response.data.ad.model);
      setFirstRegistration(
        response.data.ad.firstRegistration === 1999
          ? "Older"
          : response.data.ad.firstRegistration
      );
      setMileage(response.data.ad.mileage);
      setFuel(response.data.ad.fuel);
      setPower(response.data.ad.power);
      setPrice(response.data.ad.price);
      setDescription(response.data.ad.description);
      setAdImages(response.data.ad.images);
